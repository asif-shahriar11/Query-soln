...
JOIN 	departments DON(L.LOCATION_ID = D.LOCATION_ID)
...
WHERE	D.AVG_SALARY > ANY (
	SELECT SALARY
	FROM employees E2
);		
```

If we want to find those departments whose average salary is greater than the **minimum average salary** of all other
departments:

```sql
SELECT	D.DEPT_NAME, D.AVG_SALARY 
FROM	(
	SELECT E1.DEPARTMENT_ID DEPT_ID, E1.DEPARTMENT_NAME DEPT_NAME, AVG(SALARY) AVG_SALARY
	FROM employees E1
	GROUP BY E1.DEPARTMENT_ID
) D
WHERE	D.AVG_SALARY >  (
	SELECT MIN(SUB.AVG_SALARY)
	FROM (
        	SELECT		E2.DEPARTMENT_ID, AVG(E2.SALARY) AS AVG_SALARY
         	FROM 		EMPLOYEES E2
         	GROUP BY 	E2.DEPARTMENT_ID
	) SUB
	WHERE SUB.DEPARTMENT_ID <> D.DEPT_ID
);		
```
...
SELECT (
	SELECT	DEPARTMENT_NAME
	FROM departments D2
	WHERE D2.DEPARTMENT_ID = D.DEPT_ID
) "Department ID", D.EMP_COUNT "Number of Employees"
FROM	(
...
WHERE 	D.EMP_COUNT = (
        ...
        FROM (
            SELECT	COUNT(DISTINCT E2.EMPLOYEE_ID) AS EMP_COUNT
            FROM 	EMPLOYEES E2
            GROUP BY 	E2.DEPARTMENT_ID
        ) SUB
);
...
Another version which uses join in main query:
...
```sql
SELECT		E.JOB_ID, J.JOB_TITLE, E.LAST_NAME, MAX(E.SALARY) AS MAX_SALARY
FROM 		EMPLOYEES E
JOIN 		JOBS J ON E.JOB_ID = J.JOB_ID
GROUP BY 	E.JOB_ID, J.JOB_TITLE, E.LAST_NAME
ORDER BY 	E.JOB_ID;
```
